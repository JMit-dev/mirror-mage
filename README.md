...
## TODO
1. Highest priority: add TURN support using Open Relay by Metered and test with the free 20 GB plan so online play works on stricter networks like Stony Brook University. Default is p2p but if it doesnt connect use TURN or just make TURN default
2. Fix the online join/start security and connection bug affecting some Stony Brook University players.
3. Update the lobby so it is the only online option.
4. Add a new lava level.
5. Add new music for lava and lower all music to be less loud.
6. Add a new logo.
7. Change levels to be arena names: grass, cave and lava. 
...
### Multiple Levels
Instead of a single, custom scene class, we've defined an abstract class called `MBLevel` extending the base Scene class with two subclasses; `MBLevel1` and `MBLevel2`. The hierarchy is shown roughly in the diagram below. I have omitted the methods and fields from the diagram for the sake of keeping things simple.
...
classDiagram
  ...
  MBLevel <|-- MBLevel2
```
Most of the heavy lifting is done in the abstract MBLevel class. The subclasses will ultimately have to override and implement some functionality to get things working.
...
The transitions between the different states have been modeled after the state diagram shown below. StateMachine AI can very quickly become a tangled mess and in practice become very difficult to scale. Watch out for this as you're making your games.
...
For this assignment, you'll need to load in the custom sprite you made for homework 1. To do this, you'll need to change the sprite loaded in via the `loadScene()` method of the Level1 and Level2 scenes.

We have defined a const object with keys for the sprite in the `PlayerController.ts` file. You'll most likely want to change these keys to match up with the animation keys associated with your own sprite.
...
Once your animated sprite is loaded in and set up, you'll need to play your sprite's animations appropriately. 

* While the main character is not moving (idling), your sprite should play its `IDLE` animation
* When the main character takes damage from falling, your sprite should play its `TAKING_DAMAGE` animation
* While the main character is moving to the left, your sprite should play its `WALK_LEFT` animation if it's not already playing
* While the main character is moving to the right, your sprite should play its `WALK_RIGHT` animation if it's not already playing
* When the main character attacks, they should play an attack animation
* If the main character is facing to the right, your sprite should play its `ATTACK_RIGHT` animation.
* If the main character is facing to the left, your sprite should play its `ATTACK_LEFT` animation.
* When the main character's health hits zero, your sprite should play its `DYING` animation if it's not already playing
* After the main character's death animation has played, your sprite should play its `DEAD` animation.

## Part 2 - Creating and Playing Sound Effects
In this assignment, you will have to create and play your sound effect and level music. You may use whatever software you'd like to create your sound effect and level music, like [Audacity](https://www.audacityteam.org/) and [Chiptone](https://sfbgames.itch.io/chiptone)
...
In this assignment, you should create a custom dying sound effect for your character. The dying sound should play when your character starts playing their dying animation. 

## Part 2.2 - Playing Level Music
Create some custom-level music for the second level (MBLevel2). When the game transitions to the second level, your custom-level music should start playing.
...
## Part 4 - Tweening
Add a tween to your character's sprite to make them do a flip. The tween should rotate your character's sprite by 360 degrees. The character's flip tween should play when the character transitions from the `Walk` state to the `Jump` state.
...
The four collision groups have definitions in the `MBPhysicsGroups.ts` file shown below:
...
Inside the MBLevel class, there are two methods used to catch and handle particle collisions with the destructible layer of the tilemap.
